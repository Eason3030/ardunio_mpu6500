...
void testModules() {
  ...
  display.setTextColor(SSD1306_BLACK);             // 設定文字為黑色（反白）
  display.setTextSize(3);                          // 設定文字大小為 3
  display.setCursor(20, 6);              // 設定游標位置
  display.print("TEST");                 // 顯示 "TEST"
  display.display();                     // 更新顯示
  display.setTextColor(SSD1306_WHITE);   // 恢復文字為白色
  delay(1000);                           // 停留 1 秒

  // === 測試項目與對應 LED ===
  String tests[] = {"OLED", "IR", "MPU", "HALL", "SPEED", "RELAY", "BUZZ"};  // 測試項目名稱
  int leds[] = {LED_OLED, LED_IR, LED_MPU, LED_HALL, LED_HALL, LED_RELAY, LED_BUZZER};  // 對應 LED

  for (int i = 0; i < 7; i++) {          // 逐一測試 7 個模組
    digitalWrite(leds[i], HIGH);         // 點亮當前測試模組的 LED

    display.clearDisplay();              // 清空顯示

    // === 頂部標題框 ===
    display.setTextSize(1);              // 設定文字大小為 1
    display.drawRect(0, 0, 128, 10, SSD1306_WHITE);  // 繪製頂部框
    display.setCursor(2, 2);             // 設定游標位置
    display.print("TEST");               // 顯示 "TEST"

    // === 中間測試項目 ===
    // SPEED 和 HALL 用小字體，其他用大字體
    if (i == 3 || i == 4) {              // HALL 或 SPEED
      display.setTextSize(1);            // 小字體
      display.setCursor(30, 16);         // 調整位置（垂直置中）
      display.print(tests[i]);           // 顯示測試項目名稱
    } else {                             // 其他項目
      display.setTextSize(2);            // 大字體
      display.setCursor(30, 14);         // 設定游標位置
      display.print(tests[i]);           // 顯示測試項目名稱
    }

    // === 底部資訊列 ===
    display.setTextSize(1);              // 設定文字大小為 1
    display.drawLine(0, 26, 128, 26, SSD1306_WHITE);  // 繪製分隔線（調高位置）
    
    // 左下：笑臉
    display.setCursor(2, 28);            // 調整位置
    display.print(":)");                 // 顯示笑臉
    
    // 右下：進度
    display.setCursor(100, 28);          // 調整位置
    display.print(i+1);                  // 顯示當前測試編號
    display.print("/7");                 // 顯示總測試數
    ...
    if (i == 0) {  // === OLED 測試 ===
      ...
      // 顯示 OK（使用適當字體）
      display.setTextSize(2);            // 設定文字大小為 2
      display.setCursor(75, 14);         // 右側位置
      display.print("OK");               // 顯示 "OK"
      display.display();                 // 更新顯示
      delay(600);                        // 停留 600ms

    } else if (i == 1) {  // === IR 紅外線測試 ===
      unsigned long startWait = millis();  // 記錄開始等待時間
      bool detected = false;               // 檢測旗標

      // 等待 1 秒檢測紅外線
      while (millis() - startWait < 1000) {
        if (IrReceiver.decode()) {         // 如果接收到紅外線信號
          detected = true;                 // 設定檢測旗標
      ...
      // 顯示結果（小字體對應小字體測試項目）
      display.setTextSize(1);              // 設定文字大小為 1
      display.setCursor(90, 16);           // 右側位置（垂直對齊）
      if (hall == LOW) {                   // 如果檢測到磁鐵（低電位）
        display.print("OK");               // 顯示 "OK"
        display.display();                 // 更新顯示
      ...
      for (int j = 0; j < 2; j++) {
        ...
        digitalWrite(LED_BUZZER, LOW);     // 熄滅 LED（反向同步）
        delay(75);                         // 延遲 75ms
        digitalWrite(BUZZER_PIN, LOW);     // 關閉蜂鳴器
        digitalWrite(LED_BUZZER, HIGH);    // 點亮 LED
        delay(75);                         // 延遲 75ms
      }
    }

    delay(200);                            // 保持 LED 亮 200ms 後進入下一項
  }
  ...
  display.setTextColor(SSD1306_BLACK);               // 設定文字為黑色（反白）
  display.setTextSize(3);                            // 設定文字大小為 3
  display.setCursor(16, 6);                // 設定游標位置
  display.print("DONE");                   // 顯示 "DONE"
  display.display();                       // 更新顯示
  display.setTextColor(SSD1306_WHITE);     // 恢復文字為白色
